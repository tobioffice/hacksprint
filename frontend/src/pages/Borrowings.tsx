...
const Borrowings = () => {
  ...
  const fetchBorrowings = async () => {
    ...
    try {
      ...
      const myResponse = await axios.get('http://localhost:5000/api/borrowings/my', {
      ...
      if (userData.role === 'librarian' || userData.role === 'admin') {
        const allResponse = await axios.get('http://localhost:5000/api/borrowings', {
          headers: { Authorization: `Bearer ${token}` }
        })
  ...
  const handleReturn = async (borrowingId: string) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/borrowings/return/${borrowingId}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        {/* My Borrowings */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">My Borrowings</h2>
          {borrowings.length === 0 ? (
                ...
                {borrowings.map((borrowing) => (
                  <li key={borrowing._id} className="px-6 py-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{borrowing.bookId.title}</p>
                        <p className="text-sm text-gray-500">Author: {borrowing.bookId.author}</p>
        ...
        {(user?.role === 'librarian' || user?.role === 'admin') && (
          <div>
            <h2 className="text-xl font-semibold text-gray-900 mb-4">All Borrowings</h2>
                    ...
                    <li key={borrowing._id} className="px-6 py-4">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          {borrowing.userId && (
                            <p className="text-sm text-gray-500">
                              <span className="font-medium">Borrower:</span> {borrowing.userId.name} ({borrowing.userId.email})
                            </p>
                          )}
                          <p className="text-sm text-gray-500">
                            Borrowed: {new Date(borrowing.borrowDate).toLocaleDateString()} |
